...
export class DeterministicDeployer {
  ...
  constructor (readonly provider: JsonRpcProvider, readonly wallet?: Wallet, network?: string) {
    if (network === 'boba_mainnet') {
      ...
      this.deploymentSignerAddress = '0xdDC61b272C2120D3e017886D647427d9e65603CF'
      this.deploymentGasPrice = 1500000049
      this.deploymentGasLimit = 70000
    }
  }
  ...
  async deployDeployer (): Promise<void> {
    ...
    const rawTxResponse = await this.provider.send('eth_sendRawTransaction', [this.deploymentTransaction])
    // wait for three block confirmations
    await this.provider.waitForTransaction(rawTxResponse, 3)
    if (!await this.isContractDeployed(this.proxyAddress)) {
