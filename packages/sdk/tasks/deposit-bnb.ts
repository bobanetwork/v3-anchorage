...
task('deposit-bnb', 'Deposits BOBA and BNB onto L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    const depositBobaTx = await L1StandardBridge.depositERC20(
      L1BobaToken.address,
      l2BobaTokenAddress,
      utils.parseEther('10'),
      99999,
    ...
    const l2BobaBalance = await l2Provider.getBalance(l2Signer.address)
    if (l2BobaBalance.lt(utils.parseEther('10'))) {
      throw new Error(
    ...
    const l2BnbBalance = await L2BnbToken.balanceOf(l2Signer.address)
    if (l2BnbBalance.lt(utils.parseEther('1'))) {
      throw new Error(
        `bad deposit. recipient balance on L2: ${utils.formatEther(
          l2BnbBalance
        )}`
    ...
    const withdrawalBobaTx = await L2StandardBridge.withdraw(
      l2BobaTokenAddress,
      utils.parseEther('1'),
      99999,
      '0x',
